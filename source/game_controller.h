...
static bool validate_move(game_controller* gc, move_type move) {
	...
	if (gc->board[from_y][from_x] != piece || gc->board[to_y][to_x] != STATE_EMPTY)
		return false;
...
static void move_piece(game_controller* gc, move_type move) {
	...
	cell_state piece;
	if ((is_player_turn(gc) && gc->player_turn_parity == 0) || (!is_player_turn(gc) && gc->player_turn_parity == 1))
		piece = STATE_BLACK;
	else
		piece = STATE_WHITE;
...
static int check_wins(game_controller* gc) {
	int color_player[2];
	if (gc->player_turn_parity == 0 && is_player_turn(gc)) {
		color_player[0] = 2;
		color_player[1] = 1;
	}
	else if (gc->player_turn_parity == 1 && is_player_turn(gc)) {
		color_player[0] = 4;
		color_player[1] = 1;
	}
	else if (gc->player_turn_parity == 1 && !is_player_turn(gc)) {
	...
	for (i = 1; i < 6; i++) {
		for (j = 0; j < 6; j++) {
			if(gc->board[i][j] == color_player[0]) {
				place[k][0] = i;
				place[k][1] = j;
				k++;
			}
	...
	if(place[1][0] - place[0][0] == 0 && place[1][1] - place[0][1] == 1) {
		if(place[2][0] - place[1][0] == 0 && place[2][1] - place[1][1] == 1)
			return color_player[1];
	 	else
			return 0;
	}
	else if (place[1][0] - place[0][0] == 1 && place[1][1] - place[0][1] <= 1 && place[1][1] - place[0][1] >= -1) {
		if (place[2][0] - place[1][0] == 1 && place[2][1] - place[1][1] == place[1][1] - place[0][1])
			return color_player[1];
	 	else
			return 0;
	}
